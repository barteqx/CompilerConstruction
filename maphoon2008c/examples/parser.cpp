...
// Here come the tables:
   ...
   unsigned int starts [] = 
   {
      0, 11, 15, 18, 21, 30, 33, 42, 45, 47, 50, 
      59, 68, 73, 75, 77, 82, 87, 87, 0 };

   unsigned int defaults [] = {
      0, 1, 2, 3, 4, 5, 6, 7, 1, 9, 10, 
      11, 12, 1, 1, 15, 16, 17, 0 };
...
void reduction_0(
   ...
   std::list < token > :: iterator position,
   std::list < token > :: iterator Sign1,
   std::list < token > :: iterator Int2,
   std::list < token > :: iterator Frac3,
   std::list < token > :: iterator Exp4 ) throw( refused )
{

#line 4 "task4.m"

{ reduce( stack, position, tkn_Fp, tkn_Fp ); return; }
...
{ reduce( stack, position, tkn_Sign, tkn_Sign ); return; }
...
void reduction_4(
   std::list < token > & stack,
   std::list < token > :: iterator position,
   std::list < token > :: iterator digit1 ) throw( refused )
{

#line 12 "task4.m"

{ reduce( stack, position, tkn_Int, tkn_Int ); return; }
...
void reduction_5(
   std::list < token > & stack,
   std::list < token > :: iterator position,
   std::list < token > :: iterator Int1,
   std::list < token > :: iterator digit2 ) throw( refused )
{
...
{ reduce( stack, position, tkn_Frac, tkn_Frac ); return; }
...
void reduction_7(
   std::list < token > & stack,
   std::list < token > :: iterator position,
   std::list < token > :: iterator dot1,
   std::list < token > :: iterator Int2 ) throw( refused )
{
...
{ reduce( stack, position, tkn_Exp, tkn_Exp ); return; }
...
void reduction_9(
   ...
   std::list < token > :: iterator Sign2,
   std::list < token > :: iterator Int3 ) throw( refused )
{
   ...
   while( true ) 
   {
      ...
      while( actions. size( ) > 1 ) 
      {
         ...
         try
         {
            ...
            {
case 0:
   reduction_0( parsestack, parsestack. end( ) - 4, parsestack. end( ) - 4, parsestack. end( ) - 3, parsestack. end( ) - 2, parsestack. end( ) - 1 );
   break;
case 1:
   reduction_1( parsestack, parsestack. end( ) - 0 );
   break;
case 2:
